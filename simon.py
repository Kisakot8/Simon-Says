...
5: 'M'}
    ...
    for i in range(len(current_colours)-1):
        sleep(speed)
        if difficulty == 4:
            print(f'{colours[current_colours[i]] + random_choice(fake_colours) + Fore.RESET},      ',end='\r')
            # Need spaces at the end to clear longer colours, e.g. Yellow, and then Blue, otherwise Yellow, -> Blue,w
        else:
            print(f'{colours[current_colours[i]]},      ',end='\r')
    sleep(speed)
    if difficulty == 4:
        print(f'{colours[current_colours[-1]] + random_choice(fake_colours) + Fore.RESET}.      ')
    else:
        print(f'{colours[current_colours[-1]]}.      ')
    ...
    if simon_says:
        ...
        for i in range(len(guess)):
            if guess[i].upper() != num_to_initial_dict[current_colours[i]]:
...
def tutorial() -> None:
    input(Style.BRIGHT + '''================ SIMON SAYS ================''' + Style.RESET_ALL + '''
...
Press enter to continue...
''')
    input('''There are 4 difficulties: ''' + Fore.GREEN + '''Easy, ''' + Fore.YELLOW + '''Medium, ''' + Fore.RED + '''Hard ''' + Fore.RESET + '''and ''' + Fore.RED + Style.BRIGHT + '''Oh God''' + Style.RESET_ALL + '''. The difference between the first 3 is that
...
Chance is determined by a decimal, e.g. 0.35 = 35% chance, 1.0 = 100% chance.
(Current settings = {new_stns[2]})
''',choices_range_float=[0.0,1.0]))
    ...
    if len(choices) > 0:
        ...
        while True:
            ...
            if choice > choices_range_float[0] and choice <= choices_range_float[1]:
                ...
                print(f'Float must be between {choices_range_float[0]} and {choices_range_float[1]}')
        return choice
...
def main():
    ...
    while True:
        choice = enter_choice('''======== SIMON SAYS ========
1. Play
2. Tutorial
3. Settings
4. Hi-scores
